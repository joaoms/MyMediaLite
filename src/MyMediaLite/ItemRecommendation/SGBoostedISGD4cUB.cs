...
namespace MyMediaLite.ItemRecommendation
{
	...
	///   </para>
	/// </remarks>
	public class SGBoostedISGD4cUB : IncrementalItemRecommender, IIterativeModel
	{
		...
		protected List<double>[] user_node_lr;

		///
		public SGBoostedISGD4cUB ()
		{
			UpdateUsers = true;
			UpdateItems = true;
		...
		protected virtual void InitModel()
		{
			...
			user_node_lr = new List<double>[num_nodes];
		...
		protected virtual float Predict(int user_id, int item_id, bool bound)
		{
			...
			for (int i = 0; i < num_nodes; i++)
				if (!float.IsNaN(p = recommender_nodes[i].Predict(user_id, item_id)))
					result += user_node_lr[i][user_id] * p;
