...
namespace MyMediaLite.ItemRecommendation
{
	public class BaggedISGDOld : MF
	{
		...
		protected List<ISGDOld> recommender_nodes;
		...
		{
			base.InitModel();
			recommender_nodes = new List<ISGDOld>(num_nodes);
			ISGDOld recommender_node;
			for (int i = 0; i < num_nodes; i++) {
				recommender_node = new ISGDOld();
		...
		public override string ToString()
		{
			return string.Format(
				...
				"BaggedISGDOld num_factors={0} regularization={1} learn_rate={2} num_iter={3} incr_iter={4} decay={5} num_nodes={6} aggregation_strategy={7}",
