...
namespace MyMediaLite.ItemRecommendation
{
	...
	///   </para>
	/// </remarks>
	public class GBoostedUIISGDIter : IncrementalItemRecommender, IIterativeModel
	{
		...
		///
		protected List<uint>[] lambda_u;

		///
		protected List<int> user_counts;

		///
		public GBoostedUIISGDIter ()
		{
			UpdateUsers = true;
			UpdateItems = true;
		...
		protected virtual void InitModel()
		{
			...
			lambda_u = new List<uint>[num_nodes];
			...
			for (int i = 0; i < num_nodes; i++) {
				...
				lambda_u[i] = new List<uint>();
				for (int j = 0; j <= MaxUserID; j++)
				{
					node_err_u[i].Add(0);
					lambda_u[i].Add(NumIter);
				}
			}
		}
		...
		public override void AddFeedback(System.Collections.Generic.ICollection<Tuple<int, int>> feedback)
		{
			...
			foreach (var entry in feedback)
			{
				...
				for (int i = 0; i < num_nodes; i++)
				{
					...
					recommender_nodes[i].IncrIter = lambda_u[i][user];
					recommender_nodes[i].AddFeedbackRetrainN(new Tuple<int,int>[] {entry}, 1, target);
