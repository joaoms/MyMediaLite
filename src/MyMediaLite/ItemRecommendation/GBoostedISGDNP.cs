...
namespace MyMediaLite.ItemRecommendation
{
	...
	///   </para>
	/// </remarks>
	public class GBoostedISGDNP : IncrementalItemRecommender, IIterativeModel
	{
		...
		public override void AddFeedback(System.Collections.Generic.ICollection<Tuple<int, int>> feedback)
		{
			...
			foreach (var entry in feedback)
			{
				...
				for (int i = 0; i < num_nodes; i++)
				{
					...
					recommender_nodes[i].AddFeedbackRetrainN(new Tuple<int,int>[] {entry}, lambda, target);
