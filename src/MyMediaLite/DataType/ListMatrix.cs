...
namespace MyMediaLite.DataType
{
	...
	public class ListMatrix<T> : IMatrix<T>
	{
		/// <summary>Data array: data is stored in columns.</summary>
		protected internal List<T> data;
		...
		public ListMatrix(int dim1, int dim2)
		{
			if (dim1 < 0)
			...
			this.dim1 = dim1;
			this.dim2 = dim2;
			this.data = new List<T>(new T[dim1 * dim2]);
		}

		/// <summary>Initializes a new instance of the Matrix class</summary>
		/// <param name="dim1">the number of rows</param>
		/// <param name="dim2">the number of columns</param>
		public ListMatrix(int dim1, uint dim2)
		{
			this.dim1 = dim1;
			this.dim2 = (int) dim2;
			this.data = new List<T>(new T[dim1 * dim2]);
		...
		/// <param name="dim1">the number of rows</param>
		/// <param name="dim2">the number of columns</param>
		public ListMatrix(uint dim1, int dim2)
		{
			this.dim1 = (int) dim1;
			this.dim2 = dim2;
			this.data = new List<T>(new T[dim1 * dim2]);
		...
		/// <param name="dim1">the number of rows</param>
		/// <param name="dim2">the number of columns</param>
		public ListMatrix(uint dim1, uint dim2)
		{
			this.dim1 = (int) dim1;
			this.dim2 = (int) dim2;
			this.data = new List<T>(new T[dim1 * dim2]);
		...
		public ListMatrix(Matrix<T> matrix)
		{
			...
			this.data = new List<T>(matrix.data);
		}

		/// <summary>Copy constructor. Creates a deep copy of the given matrix.</summary>
		/// <param name="matrix">the matrix to be copied</param>
		public ListMatrix(ListMatrix<T> matrix)
		{
			this.dim1 = matrix.dim1;
			this.dim2 = matrix.dim2;
			this.data = new List<T>(matrix.data);
		...
		public ListMatrix(IList<IList<T>> data)
		{
			this.dim1 = data.Count;
			this.dim2 = data[0].Count;
			this.data = new List<T>(this.dim1 * this.dim2);
		...
		{
			return new ListMatrix<T>(num_rows, num_columns);
		}

		///
		public IMatrix<T> Transpose()
		{
			var transpose = new ListMatrix<T>(NumberOfColumns, NumberOfRows);
			for (int i = 0; i < dim1; i++)
				for (int j = 0; j < dim2; j++)
		...
		{
			return data.GetRange(i * dim2, dim2);
		}
		...
		{
			List<T> column = new List<T>(this.dim1);
		...
		public void AddRows(int num_rows)
		{
			if (num_rows > dim1)
			{
				...
				int dif = (num_rows - this.dim1) * this.dim2;
				this.dim1 = num_rows;
				this.data.AddRange(Enumerable.Repeat<T>(default(T), dif));
			}
		}
